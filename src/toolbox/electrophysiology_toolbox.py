...
def extract_lfp(raw: np.array, fs: float, 
    filter_freq: float=200, max_out_fs: float = 500, order: int = 3
    ...
    if(fs > max_out_fs):
        return lfp[::int(fs/max_out_fs)], fs/int(fs/max_out_fs)
    else:
        return lfp, fs
...
def extract_mu(
    ...
    if(fs > max_out_fs):
        return mu[::int(fs/max_out_fs)], fs/int(fs/max_out_fs)
    else:
        return mu, fs